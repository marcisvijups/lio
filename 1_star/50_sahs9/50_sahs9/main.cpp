...
int main()
{
    ...
    FILE* outfile = fopen("sahs9.rez", "w+");
	for(int i = 0; i < 8; ++i)
	{
		for(int j = 0; j < 8; ++j)
		{
 ...
 * @brief: 				uzzīmē figūras gājienu patternu izmantokjot rekursīvu funkcijas izsaukšanu
   ...
   @param [in] avoidx   lauciņa kuru nevar šķērsot vai atrasties x koordināta
   @param [in] avoidy	lauciņa kuru nevar šķērsot vai atrasties y koordināta
*/
void draw_move_pattern(uint8_t mas[8][8], uint8_t x, uint8_t y, uint8_t moves, uint8_t avoidx, uint8_t avoidy)
{
	if(mas[y][x] > moves)
